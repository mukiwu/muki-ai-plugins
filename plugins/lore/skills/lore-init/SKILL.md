...
5. **Check the glossary — but never scaffold it here.** Look for `docs/lore/glossary.md`. Per lore-spec it is created lazily, the moment the first term is agreed with the user via `lore-ul` — never templated in bulk at init time, since terms must be user-confirmed, not generated. If it's missing (true for both a fresh scaffold and an existing base you're augmenting), say so plainly in your report and suggest running `lore:lore-ul` whenever the user wants to start aligning vocabulary. This is a report line, not an action — do not create the file yourself.
