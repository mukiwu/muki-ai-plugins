...
7. **⑦ Language.** If `docs/lore/glossary.md` (or any `<area>/terms.md`) exists: check every term's meta is complete (`code:`, `updated:`, `status:`), and spot-check the codebase for `not:` names still used in domain positions — a heuristic sample judged in context, not an exhaustive lint. Also spot-check liveness the other way: does the term's `code:` type still exist, and does the term itself still appear anywhere in the codebase? A term that fails both is a dead word — a document whose language no longer appears in the code has stopped working; flag it as an `obsolete` candidate for `lore-maintain`. Fold in each term's adoption from the feedback log (keys `glossary.md#<Term>`). If there is no glossary, report "no glossary yet — `lore-ul` can bootstrap one"; that is a note, not a failure.
