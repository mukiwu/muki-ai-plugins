---
name: review-tests
description: 測試體檢診斷器——讀一份既有測試，找出盲點與建議，產出一份 self-contained HTML 報告（不修改任何 source／測試／設定）。報告每條 finding 可 toggle 展開被測 source function 的完整程式碼。補測試請改走 tdd skill。
invocation: review-tests
examples:
  - review-tests
  - review-tests <目標檔案路徑>
---
...
2. **檢查**：對照下方四類維度，逐項檢視測試內容，產出 findings。
3. **定位關鍵 source 片段**：為**每一條 finding** 找出被測的那個 `.ts/.vue/.tsx` 函式（不是 spec 裡的 test），記下：函式名稱、source 檔路徑、以及**與這條 finding 真正相關的關鍵行範圍**（通常 3～8 行，不是整個 function——展整個 function 重點會被淹沒、不好找）。
...
2. 在**專案根**執行：`node "<本 skill 目錄>/render.cjs" /tmp/<name>-findings.json`
   ...
   - 生成器會印出產出的 HTML 絕對路徑。
3. 自動開啟：macOS `open <路徑>`、Linux `xdg-open <路徑>`。
4. **提醒使用者**把 `.review-tests/` 加進 `.gitignore`（若尚未加）。
5. 終端只印一行：HTML 路徑 + 一句健康度摘要。完整內容都在 HTML，不要在終端重印整份報告。
...
- `src`：該 finding 的 backlink 定位。`fn` 是 function 名（toggle 標題用）；`from`–`to` 是要展開的**關鍵行範圍**。`file` 選填，跨檔時才填（例如 `.vue` 用到的 composable 在另一支 `.ts`）。
...
3. **測試框架**：主要針對 Vue 3 + TypeScript + Vitest，其他框架可比照原則檢查。
