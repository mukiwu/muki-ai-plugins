---
name: deps-check
description: 在編輯或重構既有檔案前，查出「誰依賴我」，避免改 A 壞 B。當使用者要求重構、改名、刪除、修改 public API、改 function signature、搬移檔案，或是動到 `src/lib/`、`utils/`、共用元件等高扇入檔案時觸發。觸發關鍵字：重構、refactor、改名、rename、刪掉、remove、修改簽名、改 API、搬檔案、move。編輯前先跑腳本列出依賴方，再決定影響範圍。
---
