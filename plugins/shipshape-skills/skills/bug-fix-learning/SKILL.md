...
Cookbook 的完整定位與三問判準見 CLAUDE.md「品質關卡 → Cookbook 同步」。以下是此 skill 適用的判斷摘要。
...
**B. 業務邏輯**
- **程式碼看不出 why 的商業規則**：感測器離線 30 分鐘後顯示 `--`（這是業務端定的，不是技術限制）
- **領域特有的計算邏輯**：良率的計算方式、報表的數據聚合規則
- **流程約束**：某個 API 必須在另一個 API 之後呼叫、某個狀態轉換有前置條件

**C. 架構決策**
- **歷史決策的 why**：為什麼這裡不用 X 而用 Y（背景決策不寫進程式會一直被重新質疑）
- **技術選型理由**：為什麼用 date-fns 不用 dayjs、為什麼這個元件不用第三方套件
- **難以自動驗證的設計規則**：Dark mode 色彩配對、品牌色用法（這類通常放 CLAUDE.md 更適合）
...
| 感測器離線 30 分鐘後顯示 `--` | ✅ | 業務規則，程式碼看不出為什麼是 30 分鐘 |
| 良率計算要排除前 10 分鐘的暖機數據 | ✅ | 領域知識，AI 不知道就會算錯 |
| 用 date-fns 不用 dayjs 因為 tree-shaking 更好 | ✅ | 架構決策，不寫會一直被重新質疑 |
| 這個 bug 改了 3 行就好 | ❌ | git log 有了 |
| Button 元件放在 src/components/ui/ | ❌ | grep 就能找到，寫了會腐爛 |
