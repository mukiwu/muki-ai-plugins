...
description: 當使用者描述程式行為與預期不符時觸發 — 不論是明確說「有 bug」，還是隱含的「為什麼會這樣」「明明應該 X 卻 Y」「這裡怪怪的」。**處理 GitHub issue 時也要先判斷**：如果 issue 內容屬於 bug（錯誤行為、非預期結果、regression、故障回報），必須呼叫此 skill；如果是 feature request、enhancement、question、docs 則不用。此 skill 強制走完五個步驟：修復 → 根因分析 → 沉澱判準 → 寫入或建議替代動作 → 輸出評估摘要。**不可在修完 bug 後直接結束回覆**，即使評估結果是「不需要寫入 cookbook」，也必須明確輸出摘要讓使用者看見此 skill 已執行。只要使用者的描述暗示「現狀 ≠ 預期」，就必須觸發此 skill。
---
...
**處理 GitHub issue 時也要先判斷是否為 bug**：

當使用者說「幫我看這個 issue」「處理 issue #123」「這張 ticket」時，先讀完 issue 內容再分類：

| Issue 類型 | 是否觸發 bug-fix-learning |
|-----------|---------------------------|
| Bug / defect / regression / 「壞了、錯了、不對」 | ✅ 必須觸發 |
| Feature request / enhancement | ❌ 不觸發，走 `/feature` 流程 |
| Question / 使用問題 | ❌ 不觸發 |
| Documentation | ❌ 不觸發 |
| Refactor / chore | ❌ 不觸發（除非順便修 bug） |

判斷依據：issue label、title 關鍵字（`bug`、`fix`、`broken`、`not working`）、內文是否描述「實際行為 vs 預期行為」。模糊時優先看內文，label 可能未被正確標註。
